...
void Window::process_camera(WebCam& webcam, Params* params, bool show_fps, bool show_detections, bool show_timing, bool lite) {
    cout << "Running VEViD on video feed from camera" << endl;

    cout << "frame width: " << webcam.get_width() << " frame height: " << webcam.get_height() << endl;
...
void Window::process_image(Mat& frame, Flags* flags, Params* params, bool show_detections) {
    ...
    Vevid vevid(frame_width, frame_height, params->S, params->T, params->b, params->G); 
    ...
    if (show_detections) {
        uchar3* d_image; 
        DetectNet net(d_image, frame_width, frame_height); 
        net.create(); 
        net.run(frame); 
    }
...
void Window::process_video(VideoCapture& vid, Flags* flags, Params* params, bool show_detections) {
    cout << "Running VEViD on input video " << flags->v_value << endl;

    vid.open(flags->v_value);
    if (!vid.isOpened()) {
        cout << "Error: Could not open video file" << flags->v_value << endl;
        exit(1);
    }

    int video_width = vid.get(CAP_PROP_FRAME_WIDTH);
    int video_height = vid.get(CAP_PROP_FRAME_HEIGHT);
    cout << "frame width: " << video_width << " frame height: " << video_height << endl;
    ...
    Vevid vevid(video_width, video_height, params->S, params->T, params->b, params->G);
    VideoWriter output;
    ...
    if (flags->w_value != nullptr) {
        ...
        int fps = 30;
        Size frame_size(video_width, video_height);
    ...
    uchar3* d_image;
    DetectNet net(d_image, video_width, video_height);
    if (show_detections) {
        net.create();
    }
