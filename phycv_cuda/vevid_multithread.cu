...
struct ImageData {
    ...
    ImageData(int count, const Mat& img) : queueFrameCount(count), image(img) {}
};
...
// Function to read images into the queue
void readImages(size_t width, size_t height) {
    cap.open(CAP_ANY);
    if (!cap.isOpened()) {
    ...
    while (true) {
        ...
        if (imageQueue.size() < 10) { // Check if the queue length is less than 10
            imageQueue.emplace(imageQueue.size(), image);
            queueCondVar.notify_one();
        }
        else {
            // Wait until the queue length decreases
            queueCondVar.wait(lock, [] { return imageQueue.size() < 10; });
        }
    }
}

// Function to process and display images from the queue
void processImages() {
...
int main() {
    ...
    thread readThread(readImages, width, height); 
    thread processThread(processImages); 
